...
VOLUME_ID=$(aws ec2 describe-volumes --region $AWS_REGION --filter "Name=tag:Name,Values=dl_vocoderft_volume" --query "Volumes[].VolumeId" --output text)
VOLUME_AZ=$(aws ec2 describe-volumes --region $AWS_REGION --filter "Name=tag:Name,Values=dl_vocoderft_volume" --query "Volumes[].AvailabilityZone" --output text)
...
if [ $VOLUME_ID ]; then
		...
		if [ $VOLUME_AZ != $INSTANCE_AZ ]; then
				SNAPSHOT_ID=$(aws ec2 create-snapshot \
						...
						--tag-specifications 'ResourceType=snapshot,Tags=[{Key=Name,Value=dl_vocoderft_volume-snapshot}]' \
				...
				VOLUME_ID=$(aws ec2 create-volume \
						...
						--volume-type gp3 \
						--tag-specifications 'ResourceType=volume,Tags=[{Key=Name,Value=dl_vocoderft_volume}]' \
		...
		chmod +x train_vocoder_ft.sh

		# Initiate training using the tensorflow_36 conda environment
		sudo -H -u ubuntu bash -c "source /home/ubuntu/anaconda3/bin/activate tensorflow2_latest_p37;pip install .; ./train_vocoder_ft.sh MURF_TASK_ID" >>/home/ubuntu/logs.txt 2>&1
fi
